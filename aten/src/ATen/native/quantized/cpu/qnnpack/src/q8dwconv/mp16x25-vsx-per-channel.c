...
void pytorch_q8dwconv_ukernel_mp16x25_per_channel__vsx(
    size_t channels,
    size_t output_width,
    const uint8_t** input,
    const void* weights,
  ...
  do {
    ...
    // Compute the first 10 elements of the filters (out of 25)
    {
      ...
      // Iterate over the inputs channels -- 16 per iteration
      size_t c = channels;
      for (; c >= 16; c -= 16) {
        ...
        const vector unsigned char vkernel_zero_point = vec_xl(
            0, &quantization_params->vsx.kernel_zero_points[channels - c]);
        ...
        const vector short vxk00_hi = vec_sub(
            (vector short)vec_mergeh(vk00, vzero), vkernel_zero_point_hi);
        const vector short vxk00_lo = vec_sub(
            (vector short)vec_mergel(vk00, vzero), vkernel_zero_point_lo);
        vacc_hi_hi = vec_add(
        ...
        const vector short vxk01_hi = vec_sub(
            (vector short)vec_mergeh(vk01, vzero), vkernel_zero_point_hi);
        const vector short vxk01_lo = vec_sub(
            (vector short)vec_mergel(vk01, vzero), vkernel_zero_point_lo);
        vacc_hi_hi = vec_add(
        ...
        const vector short vxk02_hi = vec_sub(
            (vector short)vec_mergeh(vk02, vzero), vkernel_zero_point_hi);
        const vector short vxk02_lo = vec_sub(
            (vector short)vec_mergel(vk02, vzero), vkernel_zero_point_lo);
        vacc_hi_hi = vec_add(
        ...
        const vector short vxk10_hi = vec_sub(
            (vector short)vec_mergeh(vk10, vzero), vkernel_zero_point_hi);
        const vector short vxk10_lo = vec_sub(
            (vector short)vec_mergel(vk10, vzero), vkernel_zero_point_lo);
        vacc_hi_hi = vec_add(
        ...
        const vector short vxk11_hi = vec_sub(
            (vector short)vec_mergeh(vk11, vzero), vkernel_zero_point_hi);
        const vector short vxk11_lo = vec_sub(
            (vector short)vec_mergel(vk11, vzero), vkernel_zero_point_lo);
        vacc_hi_hi = vec_add(
        ...
        const vector short vxk12_hi = vec_sub(
            (vector short)vec_mergeh(vk12, vzero), vkernel_zero_point_hi);
        const vector short vxk12_lo = vec_sub(
            (vector short)vec_mergel(vk12, vzero), vkernel_zero_point_lo);
        vacc_hi_hi = vec_add(
        ...
        const vector short vxk20_hi = vec_sub(
            (vector short)vec_mergeh(vk20, vzero), vkernel_zero_point_hi);
        const vector short vxk20_lo = vec_sub(
            (vector short)vec_mergel(vk20, vzero), vkernel_zero_point_lo);
        vacc_hi_hi = vec_add(
        ...
        const vector short vxk21_hi = vec_sub(
            (vector short)vec_mergeh(vk21, vzero), vkernel_zero_point_hi);
        const vector short vxk21_lo = vec_sub(
            (vector short)vec_mergel(vk21, vzero), vkernel_zero_point_lo);
        vacc_hi_hi = vec_add(
        ...
        const vector short vxk22_hi = vec_sub(
            (vector short)vec_mergeh(vk22, vzero), vkernel_zero_point_hi);
        const vector short vxk22_lo = vec_sub(
            (vector short)vec_mergel(vk22, vzero), vkernel_zero_point_lo);
        vacc_hi_hi = vec_add(
        ...
        const vector short vxk23_hi = vec_sub(
            (vector short)vec_mergeh(vk23, vzero), vkernel_zero_point_hi);
        const vector short vxk23_lo = vec_sub(
            (vector short)vec_mergel(vk23, vzero), vkernel_zero_point_lo);
        vacc_hi_hi = vec_add(
        ...
        // Store the partial results of the accumulators in memory
      ...
      if (c != 0) {
        ...
        // Prepare the zero points of each channel
        const vector unsigned char vkernel_zero_point = vec_xl(
            0, &quantization_params->vsx.kernel_zero_points[channels - c]);
        const vector short vkernel_zero_point_hi =
            (vector short)vec_mergeh(vkernel_zero_point, vzero);
    ...
    // Compute the second 10 elements of the filters (out of 25)
    {
    ...
    // Compute the last 5 elements of the filters (out of 25)
    {
      ...
      for (; c >= 16; c -= 16) {
        ...
        // Load the scale of each channel
        ...
        const vector float vmultiplier_hi_lo = vec_xl(
            0,
            &quantization_params->vsx.requantization_scales[channels - c + 4]);
        ...
        // Store final results from the packed vector in memory
      ...
      if (c != 0) {
        ...
        if (c > 8) {
          ...
          const vector float vmultiplier_lo_hi = vec_xl(
              0,
              &quantization_params->vsx
                   .requantization_scales[channels - c + 8]);
          const vector float vmultiplier_lo_lo = vec_xl(
              0,
              &quantization_params->vsx
                   .requantization_scales[channels - c + 12]);
        ...
        // Store the results of the remaining channels in memory
        if (c & 8) {
