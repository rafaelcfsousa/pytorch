...
void pytorch_q8avgpool_ukernel_up16x9__vsx(
  ...
  do {
    ...
    /* Iterate over 16 channels computing for every channel a window. For
     * windows smaller than 9, zeros are loaded to accumulate them into the
     * accumulators/result */
    size_t k = kc;
    while (k >= 16) {
       ...
       * store the packed vector in memory */
